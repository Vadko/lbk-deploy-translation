...
2. **TUS upload через [`tus-js-client`](https://github.com/tus/tus-js-client)** — паралельно для всіх kind'ів, 6 МБ chunks. Жоден архів не буферизується цілком у пам'яті (безпечно для архівів до 10 ГБ), а при обриві мережі клієнт продовжує з потрібного байта замість починати з нуля.
